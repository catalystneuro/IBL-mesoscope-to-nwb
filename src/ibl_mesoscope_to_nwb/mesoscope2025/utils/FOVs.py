...
def get_FOV_names_from_alf_collections(one: ONE, session: str) -> list[str]:
    """Get available FOV_names for a given session."""
...
def get_number_of_FOVs_from_raw_imaging_metadata(one: ONE, session: str) -> int:
    """Get number of available FOVs for a given session.
    It is assume that recording session 00 exists and the number of FOVs is consistent across recording sessions."""
    metadata = one.load_dataset(session, dataset="_ibl_rawImagingData.meta.json", collection=f"raw_imaging_data_00")
